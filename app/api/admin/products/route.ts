...
const draftSchema = z.object({
  ...
  price: z.coerce.number().min(1, "Укажите цену.").max(100000),
  // Необязательные: формы для них больше нет, но API их принимает — так
  // старые записи и возможный импорт извне продолжают работать.
  family: z.enum(["floral", "woody", "oriental", "fresh", "gourmand"]).optional(),
  notesTop: z.string().trim().optional(),
  notesHeart: z.string().trim().optional(),
  notesBase: z.string().trim().optional(),
